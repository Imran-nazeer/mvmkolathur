...
def contact(request):
    if request.method == "POST":
        ...
        insertvalues.subject = request.POST.get('subject')
        insertvalues.mail_id = request.POST.get('email')
        insertvalues.message = request.POST.get('comment')
        subject = str(insertvalues.subject)
        body = " Name : {name}\n Email : {email}\n Message : {message}\n" .format(name=str(insertvalues.name), email=str(insertvalues.mail_id), message=str(insertvalues.message))

        send_mail(
            subject,
            body,
            insertvalues.mail_id,
